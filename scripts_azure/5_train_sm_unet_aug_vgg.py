...
# Modèle
model = sm.Unet(BACKBONE, classes=num_cats, activation='softmax', encoder_weights=None)

model.compile('Adam', loss="categorical_crossentropy", metrics=[sm.metrics.iou_score])
...
callbacks=[
    ...
    ModelCheckpoint('cityscape-sm-unet-aug-vgg.h5', save_best_only=True, verbose=1),
    LogRunMetrics()
]
...
model.save('./outputs/cityscape-sm-unet-aug-vgg.h5')
run.upload_file('./outputs/cityscape-sm-unet-aug-vgg.h5', './outputs/cityscape-sm-unet-aug-vgg.h5')


# Enregistrement du modèle
run.register_model(model_name='cityscape-sm-unet-aug-vgg.h5', model_path='./outputs/cityscape-sm-unet-aug-vgg.h5')
